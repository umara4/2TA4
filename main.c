...
int state = 0; //Determines the state the code is in.
int move = 0; //Determines if the state can move on
int hour; // variable that locks and saves hour value from the RTC clock
int minute; // variable that locks and saves minute value from the RTC clock
int second; // variable that locks and saves seconds value from the RTC clock
int date; // variable that locks and saves date value from the RTC clock
int month; // variable that locks and saves month value from the RTC clock
int year; // variable that locks and saves year value from the RTC clock

int count; // used to count how many times the user button is pressed
...
//Init LCD
	...
	ExtBtn1_Config(); // configs the external buttons
	ExtBtn2_Config();
...
*/
 ...
 //test realtime clock	
    BSP_LCD_Clear(LCD_COLOR_WHITE);
		...
		RTC_DateStructure.WeekDay=7;   //what will happen if the date/weekday is not correct?
		RTC_DateStructure.Date=31;
		RTC_DateStructure.Year=9; //2012???  how about 1912?
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) //------------------------------------------------------------------------------------------------------------------------------
{
  ...
  {	
		...
		//saving memory location for the hour,minute and seconds
		I2C_ByteWrite(&I2c3_Handle, EEPROM_ADDRESS, memLocation,count); 	

		I2C_ByteWrite(&I2c3_Handle, EEPROM_ADDRESS, memLocation+count+1, read_RTC_TimeStruct.Hours);
		I2C_ByteWrite(&I2c3_Handle, EEPROM_ADDRESS, memLocation+count+2, read_RTC_TimeStruct.Minutes);
		I2C_ByteWrite(&I2c3_Handle, EEPROM_ADDRESS, memLocation+count+3, read_RTC_TimeStruct.Seconds);
		//^updating the most recent time
  ...
  {
		if (state == 0){
			//LCD_DisplayInt(1, 0, state); //used state to test system
			LCD_DisplayString(10,0,(uint8_t *) "Previous 2 Times"); 
			LCD_DisplayString(11,0,(uint8_t *) "HH:MM:SS");
			LCD_DisplayInt(12,0,I2C_ByteRead(&I2c3_Handle,EEPROM_ADDRESS, memLocation+count+1));	//displaying last two times the button was pressed 
			LCD_DisplayInt(12,3,I2C_ByteRead(&I2c3_Handle,EEPROM_ADDRESS, memLocation+count+2)); // and accessing from memory
			LCD_DisplayInt(12,6,I2C_ByteRead(&I2c3_Handle,EEPROM_ADDRESS, memLocation+count+3));
			LCD_DisplayInt(13,0,I2C_ByteRead(&I2c3_Handle,EEPROM_ADDRESS, memLocation+count-2));
			LCD_DisplayInt(13,3,I2C_ByteRead(&I2c3_Handle,EEPROM_ADDRESS, memLocation+count-1));
			LCD_DisplayInt(13,6,I2C_ByteRead(&I2c3_Handle,EEPROM_ADDRESS, memLocation+count));
		state = 2;
		}
		else if (state == 2){
			BSP_LCD_ClearStringLine(10);	// clearing lines 
		...
		if (state==4){
			hour =(hour+1)%24;			// when in state 4, hours are incremented each button push
			if (hour==0){ 					// when hour=0, clears the LCD space to get rid of the trailing digit
				LCD_DisplayString(4,1,(uint8_t *) " ");
			}
			LCD_DisplayInt(4,0,hour); // displays the hour increments in the clock position for the user
		} 
		if (state ==5){ 								// when in state 5, minutes are incremented each button push
			minute =(minute+1)%60;
			if (minute==0){								// when minute=0, clears the LCD space to get rid of the trailing digit 
				LCD_DisplayString(4,4,(uint8_t *) " ");
			} 
			LCD_DisplayInt(4,3,minute); // displays the minute increments in the clock position for the user
		} 
		if (state ==6){						// when in state 6, seconds are incremented each button push
			second =(second+1)%60;  
			if (second==0){						// when second=0, clears the LCD space to get rid of the trailing digit 
				LCD_DisplayString(4,7,(uint8_t *) " ");
			}
			LCD_DisplayInt(4,6,second);// displays the second increments in the clock position for the user
		} if (state ==7){ 					// when in state 7, date is incremented each button push
			date =(date+1)%32;
			if (date==0){						// date cannot equal 0, so change it to equal 1
				date=1;								// clear LCD space to get rid of trailing digit
				LCD_DisplayString(7,1,(uint8_t *) " ");
			}
			LCD_DisplayInt(7,0,date); // displays the date increments in the calendar position for the user
		} if (state ==8){						// when state=8, month is incremented each button push
			month =(month+1)%13;
			if (month==0){
				month=1;									// month cannot equal 0, so change it to equal 1
				LCD_DisplayString(7,4,(uint8_t *) " ");		// clear LCD space to get rid of trailing digit
			}
			LCD_DisplayInt(7,3,month); // displays the month increments in the calendar position for the user
		} if (state ==9){						// when state=9, year is incremented each button push
			year =(year+1)%100;
			if (year==0){								// when year=0, clear LCD space to get rid of trailing digit
				LCD_DisplayString(7,7,(uint8_t *) " ");
			}
			LCD_DisplayInt(7,6,year);			// displays the year increments in the calendar position for the user
		} 
	}  //end of PIN_1 
	...
	if(GPIO_Pin == GPIO_PIN_2)
  {
		...
		if (state ==0 && move == 1){
			LCD_DisplayString(11,3,(uint8_t *)"Changing Hours");
			HAL_RTC_GetTime (&RTCHandle, &RTC_TimeStructure, RTC_FORMAT_BIN);
			HAL_RTC_GetDate (&RTCHandle, &RTC_DateStructure, RTC_FORMAT_BIN);			// getting time and date data
			hour = RTC_TimeStructure.Hours;				// using hour, minute, second, etc. to get current time and date data and locking those values so they are not affected by the clock running in the background
			minute = RTC_TimeStructure.Minutes;
			second = RTC_TimeStructure.Seconds;
		...
		if (state ==4 && move == 1){
			BSP_LCD_ClearStringLine(11);
			LCD_DisplayString(11,0,(uint8_t *)"Changing Minutes");
			state = 5;
			move = 0;
		}
		if (state ==5 && move == 1){
			BSP_LCD_ClearStringLine(11);
			LCD_DisplayString(11,0,(uint8_t *)"Changing Seconds");
			state = 6;
			move = 0;
		}
		if (state ==6 && move == 1){
			BSP_LCD_ClearStringLine(11);
			LCD_DisplayString(11,0,(uint8_t *)"Changing Days");
			LCD_DisplayString(6,0, (uint8_t *) "DD:MM:YY");
	
			LCD_DisplayInt(7,0, read_RTC_DateStruct.Date);		// displaying calendar information
			LCD_DisplayInt(7,3, read_RTC_DateStruct.Month);
			LCD_DisplayInt(7,6, read_RTC_DateStruct.Year);
			state = 7;
			move = 0;
		}
		if (state ==7 && move == 1){
			BSP_LCD_ClearStringLine(11);
			LCD_DisplayString(11,0,(uint8_t *)"Changing Months");
			state = 8;
			move = 0;
		}
		if (state ==8 && move == 1){
			BSP_LCD_ClearStringLine(11);
			LCD_DisplayString(11,0,(uint8_t *)"Changing Years");
			state = 9;
			move = 0;
		}
		if (state == 9 && move ==1){
			...
			state = 0;
			move =0;
			RTC_TimeStructure.Hours=hour;						// reassigning the RTC values to the values that were input by the user
...
{
	//LCD_DisplayInt(1, 0, state);
 	 /*else {
		LCD_DisplayInt(4,0,hour);
		LCD_DisplayInt(4,3,minute);
		LCD_DisplayInt(4,6,second);
	}*/

	
	
		if((BSP_PB_GetState(BUTTON_KEY))==1 && state == 0){//when user presses user button, display calendar information
		LCD_DisplayString(6,0, (uint8_t *) "DD:MM:YY");

		LCD_DisplayInt(7,0, read_RTC_DateStruct.Date);
		LCD_DisplayInt(7,3, read_RTC_DateStruct.Month);
		LCD_DisplayInt(7,6, read_RTC_DateStruct.Year);
		state=1;
		}
	else if(state == 1 && BSP_PB_GetState(BUTTON_KEY)==0){
		state = 0;
		BSP_LCD_ClearStringLine(6); 		// clears the lines when the user stops pressing the button
		BSP_LCD_ClearStringLine(7);
	}
	
	
	LCD_DisplayString(3,0, (uint8_t *) "HH:MM:SS");
	if (state==0||state==1||state==3){			// clock only runs when in states 0,1 or 3 so that when user is changing values, the clock does not create confusion
		RTC_TimeStructure.Seconds = (RTC_TimeStructure.Seconds+1)%60;			// seconds increment by 1 each time and using mod 60, can continously loop
		if (RTC_TimeStructure.Seconds == 0){			// when seconds =0, minutes increments by 1
			RTC_TimeStructure.Minutes = (RTC_TimeStructure.Minutes +1)%60;
			LCD_DisplayString(4,7,(uint8_t *)"  ");		// clears the trailing second digit from LCD
		}
		if (RTC_TimeStructure.Minutes==0){		// minute=0, hour goes up by 1
			RTC_TimeStructure.Hours = (RTC_TimeStructure.Hours+1)%24;
			LCD_DisplayString(4,4,(uint8_t *)"  "); // clears the trailing minute digit from LCD
		}
		if (RTC_TimeStructure.Hours ==0){			// when hour gets to 0, date goes up by 1
			RTC_DateStructure.Date = (RTC_DateStructure.Date+1)%31;
			LCD_DisplayString(4,1,(uint8_t *)"  "); // clears the trailing hour digit from LCD
		}
		if (RTC_DateStructure.Date == 0){			// when date=0, month  goes up by 1
			RTC_DateStructure.Date=1;				// there is no 0th of the month, so change the date to 1
			RTC_DateStructure.Month =(RTC_DateStructure.Month+1)%12;
			BSP_LCD_ClearStringLine(7);		// clears entire line
		} 
		if (RTC_DateStructure.Month ==0){		// if month=0,year goes up by 1 
			RTC_DateStructure.Month=1;			// no 0th month, so change to 1
			RTC_DateStructure.Year=(RTC_DateStructure.Year+1)%100;
		}
	}
	HAL_RTC_GetTime(&RTCHandle, &read_RTC_TimeStruct, RTC_FORMAT_BIN);
	HAL_RTC_GetDate(&RTCHandle, &read_RTC_DateStruct, RTC_FORMAT_BIN); 
	if (state==0||state==1||state==2||state==3){		// only display clock when state=0,1,2 or 3 so it does not interrupt when user changing values
		LCD_DisplayInt(4,0,read_RTC_TimeStruct.Hours);
		LCD_DisplayInt(4,3,read_RTC_TimeStruct.Minutes);
		LCD_DisplayInt(4,6,read_RTC_TimeStruct.Seconds);
	}
